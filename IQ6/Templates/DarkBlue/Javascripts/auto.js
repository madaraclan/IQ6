...
(function($){
	...
	function showList(data,list, img, fullname,text){
		if( !data.listVisible ){
		...
		var html = "";
		var regEx = new RegExp("("+text.toLowerCase().substr(1)+")"); // + 1 // Default = 0
		
		//var regExFullName = new RegExp("("+fullname.toLowerCase().substr(1)+")");
		
		// coba2
		console.log(data.listVisible);
		...
		for( var i=0; i< list.length; i++ ){
			...
			var fullname = fullname[i].toLowerCase();
			var $d = '';
			if (i == 0) $d = "data-selected='true'";
			
			html += "<li "+$d+" data-value='@"+list[i]+"' "+width+">"+img[i]+" <span>@"+name.replace(regEx,"<mark>$1</mark>")+"</span> <span>"+fullname.replace(regEx, "<mark>$1</mark>")+"</span></li>";
		}
		$(data.list).html(html);
	}	
	...
	function registerEvents(data){
		...
		$(data.ta).keyup(function(e){
			...
			//console.log(text.charAt(text.indexOf('@') + 1));
			//console.log("getWords return ",text);
			if( text != "" && text.charAt(text.indexOf('@') + 1) != "" ){
				data.on.query(text,function(list, img, fullname){
					
					// coba2
					//console.log("got list = ",list);
					
					if( list.length ){
						showList(data,list, img, fullname ,text);	
					}
					else{
						hideList(data);
					}
