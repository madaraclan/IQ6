...
class AES {
	...
	const WORD_LENGTH 	= 4;
	const BYTE_LENGTH	= 8;
	public function __construct($strength=self::AES256) {
		switch($strength) {
			case self::AES256:
				$this->keySize = 8;
                $this->blockSize = 4;
	...
	public function Encrypt($content, $key) {
		...
		for ($round = 1; $round < $this->numberOfRounds; $round++) {
			$this->SubBytes();
			$this->ShiftRows();
			$this->MixColumns();
		...
		$this->SubBytes();
		$this->ShiftRows();
		$this->AddRoundKey($this->CreateRoundKey($this->numberOfRounds, $expand_key));
	...
	public function Decrypt($content, $key) {
		...
		$this->state = $this->ConvertToWords($content);
		$this->AddRoundKey($this->CreateRoundKey($this->numberOfRounds, $expand_key));
		for ($round = $this->numberOfRounds - 1; $round > 0; $round--) {
			$this->InvShiftRows();
			$this->InvSubBytes();
			$this->AddRoundKey($this->CreateRoundKey($round, $expand_key));
			$this->InvMixColumns();
	...
	private function MixColumns() {
		...
		for ($i=0; $i < $this->blockSize; $i++) {
			...
			for ($j = 0; $j < self::WORD_LENGTH; $j++) {
				$byte = $mul2[$j] ^ $mul1[($j+3) % $this->blockSize] ^ $mul1[($j+2) % $this->blockSize] ^ $mul3[($j+1) % $this->blockSize];
	...
	private function InvMixColumns() {
		...
		for ($i=0; $i < $this->blockSize; $i++) {
			...
			for ($j = 0; $j < self::WORD_LENGTH; $j++) {
				$byte = $mulE[$j] ^ $mul9[($j+3) % $this->blockSize] ^ $mulD[($j+2) % $this->blockSize] ^ $mulB[($j+1) % $this->blockSize];
				$this->state[$i] = $this->PutByteIntoWord( $byte, $this->state[$i], $j);
			}
		}
	}
	...
	private function KeyExpansion($key) {
		...
		for ($i=$this->keySize; $i<$this->blockSize * ($this->numberOfRounds + 1); $i++) {
			$temp = $expanded_key[$i - 1];
			if ($i % $this->keySize == 0)
				$temp = $this->SubWord($this->RotWord($temp)) ^ $this->roundConstants[$i / $this->keySize -1];
			else if ($this->keySize > 6 && $i % $this->keySize == 4)
				$temp = $this->SubWord($temp);
			$expanded_key[$i] = $expanded_key[$i - $this->keySize] ^ $temp;
		}
		return $expanded_key;
	}
	...
	private function SubWord($word, $invers=false) {
		if (!$invers)
			return 	$this->sBox[(( $word >> 24 ) & 0x000000FF)] << 24 |
					$this->sBox[(($word & 0x00FF0000) >> 16)] << 16 |
					$this->sBox[(($word & 0x0000FF00) >> 8 )] << 8 |
					$this->sBox[($word & 0x000000FF)];
		else
			return 	$this->sBoxInverted[(( $word >> 24 ) & 0x000000FF)] << 24 |
					$this->sBoxInverted[(($word & 0x00FF0000) >> 16)] << 16 |
					$this->sBoxInverted[(($word & 0x0000FF00) >> 8 )] << 8 |
					$this->sBoxInverted[($word & 0x000000FF)];
	...
	private function ConvertToWords($content) {
		$words = array();
		for ($i = 0; $i < strlen($content); $i+=2*$this->blockSize){
			$words[$i/(2*$this->blockSize)] = hexdec(substr($content, $i, 2*$this->blockSize));
		}
		return $words;
	}
	...
	public function StringToHex($str) {
		$hex="";
		$zeros = "";
		$len = 2 * self::WORD_LENGTH * $this->blockSize;
	...
	public function SelfTest() {
		...
		for ($round = 2; $round < $this->numberOfRounds; $round++) {
			$this->SubBytes();
			$this->ShiftRows();
			$this->MixColumns();
			$this->AddRoundKey($this->CreateRoundKey($round, $expand_key));
		...
		$time1 = microtime(true);
		$this->AddRoundKey($this->CreateRoundKey($this->numberOfRounds -1, $expand_key));
		$time2 = microtime(true);
		print('Add round key time: '.(($time2 - $time1)*1000).' ms<br />');
		...
		for ($round = $this->numberOfRounds - 2; $round > 0; $round--) {
			$this->InvShiftRows();
			$this->InvSubBytes();
			$this->AddRoundKey($this->CreateRoundKey($round, $expand_key));
			$this->InvMixColumns();
