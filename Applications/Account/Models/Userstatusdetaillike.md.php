...
Import::Entity('Account.Userstatusdetaillike');

class Userstatusdetaillike extends Model{
	
	public function __construct() {
        parent::__construct();
        $this->tableName = 'userstatusdetaillike';
        $this->lastTable = 'userstatusdetaillike';
        $this->application = 'Account';
    }	
    
    public $YouLike = false;
    
    function CekLikeComment($IdStatus){
    	$q = $this
    			->Equal('USDID', $IdStatus)
    			->Get();
    	$Num = count($q);
    ...
    function LikeStatus(Input $Input){
    	...
    	if ($NumUName){
    		...
    		$DataUserUpdate = new UserstatusdetaillikeEntity();
			$DataUserUpdate->USDID = $IdStatus;
			$DataUserUpdate->UName = $UName;
			$this->Add($DataUserUpdate);
    	}
    	
    	$res = $this->CekLikeComment($IdStatus);
